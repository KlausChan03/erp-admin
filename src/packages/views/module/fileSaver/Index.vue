<template>
    <bag-view>
        <div>
            <a-input-search
                v-model:value="action.text.value"
        ...
        <div>
            <a-input-search
                v-model:value="action.site.value"
                placeholder="Hello, world!"
                size="large"
            />
            <br />
            ...
            <span>
                在相同来源内使用URL只会使用a[download]。否则，它将首先检查它是否支持带有同步头请求的cors标头。如果是这样，它将下载数据并使用Blob
        ...
        <div>
            <a-input-search
                v-model:value="action.file.value"
                placeholder="Hello, world!"
                size="large"
            />
            <br />
    ...
    setup() {
        ...
        const action = reactive({
            ...
            file: {
                value: 'Hello, world!',
                handleSave: () => {
                    const file = new File([action.file.value], 'hello world.txt', {
